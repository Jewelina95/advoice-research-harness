...
这次我们把 ADvoice 从一个内部实验工程整理成了可以交给审稿人直接检查的研究演示。打开网页以后，可以看到两个部分。第一个部分是单条录音分析。我们放入了临床访谈、图片描述、结构化认知任务和自然讲话四种输入类型。选择案例并点击运行后，系统会重新完成任务识别、音频与文本测量、指标证据构建和认知状态聚合，然后生成医生审查界面。医生看到的不只是一个分类结果，还可以继续查看每个认知状态由哪些指标支持、指标来自哪一段录音，并直接定位到相应的音频和文字片段。

第二个部分是单一数据集的队列结果。当前选择 ADReSS 2020，是因为它具有清楚的图片描述任务和受试者独立测试划分。网页展示的是 ADvoice 自身的一次归档结果，不混入开发基线。这个队列包含 27 名受试者，当前归档结果的准确率为 0.852，宏平均 F1 为 0.850，宏平均 AUROC 为 0.978。页面同时展示混淆矩阵、健康对照和 AD 两类的敏感度、精确率、特异度，以及证据对象和报告权限是否完整。这里的数字用于展示系统怎样报告一个完整实验，不应被描述成新的确认性临床验证。

从方法上看，旧版系统虽然已经提取了很多声学和语言指标，但预测链与医学解释链仍然相对分离：分类模型使用一组特征形成概率，解释模块再从已有指标中选择内容写报告。这样做的问题是，报告中的证据不一定真正参与了预测，局部片段、任务差异和反证也很难对最终判断产生可验证的影响。

新版首先增加了任务和角色路由。同样的停顿，在图片描述、语义流畅性任务和自由讲话中的含义并不相同，因此系统先确定任务、语言、可用模态和患者有效语音，再决定哪些指标在当前病例中可以使用。第二个升级是把原始指标转换成 MetricEvidence。一个静音比例不再只是一个数值，它还记录参考范围、异常方向、当前测量是否可靠、可能受到哪些混杂影响、适用于哪个任务，以及来自哪个音频片段。这样，进入预测的证据和进入医生报告的证据都有明确边界。

第三个升级是认知状态层。系统不会让多个高度相关的停顿指标重复投票，而是先把它们合成为“停顿与流畅性”等认知状态；同时保留任务特异状态，避免在融合前把图片描述、回忆和流畅性任务平均掉。第四个升级是片段轨迹。系统保留异常在整段语音中的位置和变化，因此报告中的词汇提取困难、信息密度下降或停顿负担，可以回到具体片段核查，而不是只看病例级总分。

第五个升级是受约束的单一诊断 Agent。Agent 接收的是盲化结构化证据包，而不是监督模型已经给出的答案。它需要同时检查支持证据、反证、任务适用性、混杂和证据 ID，并对认知状态作出保留、降权、失效或不可观察的更新。Agent 的输出还要经过覆盖率、可靠度、证据 ID 和开发集门禁；条件不满足时系统自动回退到监督先验。最终类别和概率锁定以后，才生成医生报告，所以报告生成不能反过来改变预测。

这套设计的主要价值不是简单增加一个大模型，而是让预测、证据和报告使用同一套可审查对象。当前已经成立的是：认知证据表示相对于相同编码器的普通融合带来了独立增益，证据 ID、报告权限和质量分离可以被程序检查。尚未成立的是诊断 Agent 对分类准确率的稳定净增益，以及相对 SpeechCARE 的总体优效。因此目前可以汇报系统结构、可追溯性和已有归档结果，但不能把仍待验证的比较写成已经超过现有方法。
