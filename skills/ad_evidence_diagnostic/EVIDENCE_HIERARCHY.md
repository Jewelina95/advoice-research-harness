...
3. **Traceability and permission**: every prediction contribution resolves to
   inference-permitted MetricEvidence. Every clinician-facing claim additionally
   requires report permission and source-segment provenance.
...
- `clinical_support`: interpretable, observable evidence that may influence
  hidden ordinal inference when inference-permitted; it may appear in the
  clinical rationale only when separately report-permitted.
